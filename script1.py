...
for nr_hz in [1, 2, 5, 10, 20]:
    nr = nr_hz * Hz
    rs = sim.run_experiment(nr, use_husain=False, record_traces=False, seed_val=42)
    rh = sim.run_experiment(nr, use_husain=True,  record_traces=False, seed_val=42)
    ...
    print(f'  {nr_hz:2d}Hz  NVM_STDP={rs["nvm_wear"]:.2f}  NVM_H={rh["nvm_wear"]:.2f}  '
          f'ratio={nvm_r:.1f}x  ent_ratio={ent_r:.1f}x  '
          f'sep_STDP={sep_s:.4f}  sep_H={sep_h:.4f}')

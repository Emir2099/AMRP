...
─── FORMAL LEARNING RULE ──────────────────────────────────────────────────────
  ...
  AMRP Factor (smooth sigmoid gate):
...
#     noise=5 Hz  → STM rate≈12 Hz, ρ_ss≈1.17  (gate partial,     H_R≈0.42 analytical / 0.45 measured)
...
def run_experiment(
    ...
    n_write_events = float(np.sum(syn.n_events[:]))
...
def snr_sweep() -> tuple[list, list, list]:
    """
    Run both groups (STDP / AMRP) across all NOISE_RATES.
    Returns (stdp_results, amrp_results, noise_values_hz).
    """
    s_list, a_list = [], []
    print('\n── SNR sweep ───────────────────────────────────────────────────')
    for nr in NOISE_RATES:
        ...
        s_list.append(run_experiment(nr, use_amrp=False, record_traces=False))
        print('✓  amrp...', end='', flush=True)
        a_list.append(run_experiment(nr, use_amrp=True,  record_traces=False))
        print('✓')
    return s_list, a_list, [float(nr / Hz) for nr in NOISE_RATES]
...
def fig_timeseries(res_h: dict, res_s: dict, noise_label: str = '20 Hz'):
    ...
    Three-panel canonical figure for manuscript:
      ...
      Panel C — Synaptic weight evolution: AMRP locks on, STDP diverges
    """
    _style()
    ...
    ax1.set_title(
        f"AMRP vs Standard STDP  ·  Background noise: {noise_label}",
    ...
    ax3.legend(
        handles=[
            Patch(facecolor=C['wh'], alpha=0.8,
                  label="AMRP — converges to signal"),
...
def fig_snr_curves(s_res: list, h_res: list, noise_vals: list):
    ...
    fig.suptitle("SNR Sweep — Standard STDP vs AMRP",
...
if __name__ == '__main__':
    ...
    rh = run_experiment(5*Hz, use_amrp=True,  record_traces=True,  verbose=True)
    rs = run_experiment(5*Hz, use_amrp=False, record_traces=True,  verbose=True)

    print(f'\n  A/B at 5 Hz noise:')
    print(f'    NVM wear  STDP={rs["nvm_wear"]:.1f}  AMRP={rh["nvm_wear"]:.1f}'
          f'  ({rs["nvm_wear"]/(rh["nvm_wear"]+1e-9):.1f}x reduction)')
    print(f'    Entropy   STDP={rs["weight_entropy"]:.5f}  AMRP={rh["weight_entropy"]:.5f}')
